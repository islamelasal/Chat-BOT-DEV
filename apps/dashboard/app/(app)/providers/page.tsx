...
export default function ProvidersPage() {
  const { data: providers, error } = useApi<any[]>('/providers');

  if (error) return <PageError msg={error} />;
  if (!providers) return <PageLoading />;
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <div className="grid gap-2 p-4 text-[11px] sm:grid-cols-2 lg:grid-cols-3">
          ...
          ] as Array<[string, string, boolean]>).map(([name, url, free]) => (
            <div key={name} className="flex items-center justify-between rounded-lg border border-slate-100 bg-slate-50 px-3 py-2">
