...
## 👑 شخصيات البوتات (Bot Personas)

- **كنز الشوا v2** ✅ البوت الافتراضي: البياع المصري النصوح (1973) — قاموس موسع (نورتنا يا فندم، عينيا ليك، ست الكل، عرض ما يتفوتش)، بروتوكول العرائس الكامل، 3 أمثلة بيع مكمل، **توجيه بالموديلات المجانية فقط** (llama-3.3-70b:free → gemma-3-12b:free → deepseek-v3:free → ديمو)، تأريض بأول 8 منتجات (أولوية الصفقات والأحدث)، **مطابقة بتطبيع عربي كامل** (تشكيل/همزات/ڤ↔ف)، **حاسبة ميزانية العروسة 👰** (65,000 ج.م + 3 أساسيات بأسعار حقيقية + إعادة حساب ديناميكي)، قراءة صوتية 🔊، ورد احتياطي + منتجين تلقائياً.
- **سلسلة فيد v4 (جاهزة للفيد الحقيقي)**: محلل **Google Shopping RSS كامل** (وسوم `g:*` + فك `CDATA` + العملة من "64.00 EGP" + فواصل الآلاف "1,252.00" + المقاسات/الألوان/الجنس كسمات `attrs` + نفاد المخزون) — اختُبر على نموذج فيد الشوا الحرفي ✓ · متغيرات عنوان (www/بروتوكول) + رؤوس فيد + محرك Googlebot · **معالجة غير متزامنة (Async Jobs)**: رفع حتى 50MB (حد الجسم 64MB — حل الـ 413) والاستجابة فورية مع شريط تقدم حي في اللوحة · **معاملات + خريطة مسبقة**: 40,000 منتج في أقل من ثانية (كانت 32 ثانية) · حد 100,000 منتج/فيد · **رفع من الجهاز** (زر 📤).
- **ملاحظة بيئة الاختبار**: الشبكة الخارجية محجوبة في صندوق الرمل هنا — في الإنتاج (VPS) السلسلة الكاملة تعمل على الفيد الحقيقي، وحالة `simulated` تظهر بوضوح كأرقام تقريبية حتى نجاح المزامنة التالية.
- **مفتاح OpenRouter**: يُثبَّت من بيئة التشغيل (`OPENROUTER_API_KEY`) أو من اللوحة — مشفر AES-256-GCM ولا يظهر أبداً.
- **مُثبّت حي (idempotent)**: `ensureKanzBot` يعمل عند كل إقلاع — تحديث `packages/db/src/kanz.ts` ينعكس على أي قاعدة بيانات دون فقد بيانات.
- كل بوت له: `is_default` + `welcome_msg` + `suggestions_json` + `fallback_msg` — قابلة للتحرير من محرر الشخصية في اللوحة، وجاهزة لأي شخصيات بوتات جديدة.
...
## 📦 تتبع الطلبات + llms.txt (تكامل CS-Cart 4.20.1 العميق)

- **تتبع الطلبات لحظياً**: إعداد حساب بوت في CS-Cart (`Administrators→API access`) → يُحفظ مشفراً في بيانات العميل → زر "📦 تتبع طلبك لحظياً" في الودجت يستدعي `GET /api/orders/{id}` بBasic Auth → خريطة حالات عربية → رد بنبرة كنز الشوا + تحقق إيميل اختياري (لا تتبع طلبات الآخرين).
- **استيراد llms.txt**: زر "📄 استيراد llms.txt" في محرر المعرفة — يحول أقسام الملف (Website→SEO→llms.txt) لشظايا معرفة تلقائياً.
- **قرار النشر محسوم**: [docs/DEPLOYMENT.md](./docs/DEPLOYMENT.md) — معاينة Arena للعرض الآن → VPS+Docker للإنتاج (جاهز) + بديل Railway/Render + قائمة Go/No-Go + فحص جاهزية المزودين `scripts/check-providers.mjs`.
...
- 🕸️ **التدريب التلقائي من روابط الموقع**: ألصق روابط العميل → استخراج نص وتقسيم تلقائي لشظايا معرفة (من صفحة البوت → قاعدة المعرفة) + **خدمة اتصال جانبية v2** مبنية على تقنيات Scrapling المدروسة من مصدرها (بصمة TLS عشوائية، سلم محركات بتصعيد عند الحجب، حل Cloudflare Turnstile، خنق تلقائي لكل دومين يحترم Retry-After، جلسات دائمة) + جلب مباشر محسّن (فك gzip + ترميز windows-1256 + كشف الحجب + إعادة محاولة بتراجع عشوائي) + **حارس SSRF بالـ DNS Pinning** — التفاصيل: [docs/SCRAPLING.md](./docs/SCRAPLING.md)
