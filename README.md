...
- **سلسلة فيد v3 (تقنيات وصول محسّنة)**: متغيرات عنوان تلقائية (www↔بدون www، http↔https) + تنظيف كيانات HTML (`&amp;`) + رؤوس فيد واقعية (RSS/XML/CSV) + **محرك Googlebot** إضافي في سلم Scrapling (http→stealth→googlebot→urllib) + مباشر 8ث → CORS proxy → **محاكاة شفافة** (sync_status=simulated موثقة بلون تحذير في اللوحة) + **رفع فيد من الجهاز** (زر 📤 في صفحة الكتالوج ≤8MB — قراءة محلية وتحليل فوري).
