...
const state = {
  ...
  arenaFiles: [],
  arenaArtifacts: null,
  arenaCapture: { commands: true, executions: true, thinking: false },
  arenaBusy: false,
  activeTabId: null,
...
function setSettingsLayer(open) {
  ...
  if (open) {
    ...
    $('#quality-mode').checked = state.settings.qualityMode;
...
function renderAiBridge() {
  ...
  const arenaTools = $('#arena-tools');
  const arenaModeLabel = $('#arena-mode-label');
  const arenaFileStatus = $('#arena-file-status');
  ...
  const isArena = chat?.provider === 'arena';
  if (arenaTools) arenaTools.hidden = !isArena || !chat?.supported;
  if (arenaModeLabel) arenaModeLabel.textContent = chat?.arenaModeLabel || 'Arena Mode';
  if (arenaFileStatus) arenaFileStatus.textContent = state.arenaBusy
    ? 'جاري الرفع…'
    : state.arenaFiles.length
      ? `${state.arenaFiles.length} ملف جاهز للإرسال`
      : 'يمكن سحب الملفات إلى المنطقة أدناه.';
  const commandsToggle = $('#arena-include-commands');
  const executionsToggle = $('#arena-include-executions');
  const thinkingToggle = $('#arena-include-thinking');
  if (commandsToggle) commandsToggle.checked = state.arenaCapture.commands;
  if (executionsToggle) executionsToggle.checked = state.arenaCapture.executions;
  if (thinkingToggle) thinkingToggle.checked = state.arenaCapture.thinking;
...
async function importAiChat({ silent = false } = {}) {
  ...
  project.externalChat = {
    ...
    artifacts: chat.provider === 'arena' ? filteredArenaArtifacts(chat.artifacts) : null,
    updatedAt: Date.now()
  };
  project.externalChats = state.additionalAiChats.map((item) => ({
    provider: item.provider,
    providerLabel: item.providerLabel,
    url: item.url,
    title: item.title,
    messages: item.messages,
    artifacts: item.provider === 'arena' ? filteredArenaArtifacts(item.artifacts) : null,
    updatedAt: Date.now()
  }));
...
async function collectOpenAiChats() {
  if (!hasChrome) return;
  try {
    const tabs = await chrome.tabs.query({ currentWindow: true });
    const chats = [];
    for (const tab of tabs.filter((item) => /^https?:\/\//i.test(item.url || '')).slice(0, 8)) {
      try {
        let chat;
        try {
          chat = await chrome.tabs.sendMessage(tab.id, { type: 'AI_CHAT_GET_STATE' });
        ...
        if (chat?.supported) chats.push(chat);
      } catch {
        // Skip protected or non-chat tabs.
      }
    }
    state.additionalAiChats = chats.filter((chat) => chat.url !== state.aiChatState?.url).slice(0, 6);
    const project = activeProject();
    project.externalChats = state.additionalAiChats.map((chat) => ({
      provider: chat.provider,
      providerLabel: chat.providerLabel,
      url: chat.url,
      title: chat.title,
    ...
    await persist();
    showToast(state.additionalAiChats.length ? `تم دمج ${state.additionalAiChats.length} محادثات AI إضافية.` : 'لم نجد محادثات AI إضافية قابلة للقراءة.', { error: !state.additionalAiChats.length });
  } catch (error) {
    showToast(`تعذر دمج تبويبات AI: ${error.message}`, { error: true });
  }
}

const ARENA_MAX_FILE_BYTES = 40 * 1024 * 1024;
const ARENA_TRANSPORT_CHUNK_BYTES = 512 * 1024;
...
async function sendFileToArena(file, name = file.name, type = file.type) {
  const uploadId = `arena-upload-${crypto.randomUUID?.() || `${Date.now()}-${Math.random()}`}`;
  const totalChunks = Math.ceil(file.size / ARENA_TRANSPORT_CHUNK_BYTES);
  const start = await chrome.tabs.sendMessage(state.activeTabId, {
    type: 'AI_CHAT_ATTACH_FILE_START', uploadId, name, type, size: file.size, totalChunks
  });
  if (!start?.ok) throw new Error(start?.reason || 'تعذر بدء رفع الملف.');
  for (let index = 0; index < totalChunks; index += 1) {
    const buffer = await file.slice(index * ARENA_TRANSPORT_CHUNK_BYTES, Math.min(file.size, (index + 1) * ARENA_TRANSPORT_CHUNK_BYTES)).arrayBuffer();
    const result = await chrome.tabs.sendMessage(state.activeTabId, {
      type: 'AI_CHAT_ATTACH_FILE_CHUNK', uploadId, index, data: arrayBufferToBase64(buffer)
    });
    if (!result?.ok) throw new Error(result?.reason || `تعذر رفع الجزء ${index + 1}.`);
    if (index % 8 === 0) {
      const percent = Math.round(((index + 1) / totalChunks) * 100);
      $('#arena-file-status').textContent = `جاري رفع ${name} · ${percent}%`;
    }
  }
  const end = await chrome.tabs.sendMessage(state.activeTabId, { type: 'AI_CHAT_ATTACH_FILE_END', uploadId });
  if (!end?.ok) throw new Error(end?.reason || 'لم يقبل Arena الملف بعد اكتماله.');
}

async function attachFilesToArena(fileList) {
  if (state.aiChatState?.provider !== 'arena') {
    showToast('افتح Arena AI في التبويب النشط أولاً.', { error: true });
    return;
  }
  ...
  try {
    ...
    for (const file of selected) {
      if (file.size <= ARENA_MAX_FILE_BYTES) queued.push({ file, name: file.name, type: file.type });
      else {
        const parts = Math.ceil(file.size / ARENA_MAX_FILE_BYTES);
        for (let part = 0; part < parts; part += 1) {
          const start = part * ARENA_MAX_FILE_BYTES;
          const end = Math.min(file.size, (part + 1) * ARENA_MAX_FILE_BYTES);
          queued.push({
            file: file.slice(start, end, file.type),
            name: `${file.name}.part-${String(part + 1).padStart(3, '0')}-of-${String(parts).padStart(3, '0')}`,
            type: file.type
          });
        }
        showToast(`تم تقسيم ${file.name} إلى ${parts} أجزاء بسبب تجاوز 40MB.`);
      }
    }
    for (const item of queued) {
      $('#arena-file-status').textContent = `جاري رفع ${item.name}…`;
      await sendFileToArena(item.file, item.name, item.type);
    }
    state.arenaFiles = [...state.arenaFiles, ...queued.map((item) => ({ name: item.name, size: item.file.size }))].slice(-12);
    showToast(`تم إرسال ${queued.length} ملفات إلى Arena AI.`);
  } catch (error) {
    showToast(`تعذر إرفاق الملفات: ${error.message}`, { error: true });
  } finally {
...
function filteredArenaArtifacts(artifacts = state.arenaArtifacts || state.aiChatState?.artifacts || {}) {
  return {
    commands: state.arenaCapture.commands ? (artifacts.commands || []) : [],
    executions: state.arenaCapture.executions ? (artifacts.executions || []) : [],
    thinking: state.arenaCapture.thinking ? (artifacts.thinking || []) : [],
    workspaceFiles: artifacts.workspaceFiles || [],
    capturedAt: artifacts.capturedAt || Date.now()
  };
}

async function refreshArenaArtifacts() {
  if (state.aiChatState?.provider !== 'arena' || !state.activeTabId) {
    showToast('افتح Arena AI في التبويب النشط أولاً.', { error: true });
    return;
  }
  try {
    const result = await chrome.tabs.sendMessage(state.activeTabId, { type: 'AI_CHAT_GET_ARTIFACTS' });
    if (!result?.ok) throw new Error(result?.reason || 'تعذر جمع أدوات Arena.');
    state.arenaArtifacts = result.artifacts;
    state.aiChatState.artifacts = result.artifacts;
    showToast(`تم جمع ${result.artifacts.commands.length + result.artifacts.executions.length} عناصر من كونسول Arena.`);
  } catch (error) {
    showToast(`تعذر جمع أدوات Arena: ${error.message}`, { error: true });
  }
}

function arenaExportData() {
  const chat = state.aiChatState || {};
  const project = activeProject();
  return {
    exportedAt: new Date().toISOString(),
    project: { name: project?.name || '', brief: project?.brief || '' },
    provider: { name: chat.providerLabel || 'Arena AI', mode: chat.arenaModeLabel || chat.arenaMode || '', url: chat.url || '' },
    messages: chat.messages || [],
    artifacts: filteredArenaArtifacts(chat.artifacts),
    attachedFiles: state.arenaFiles
  };
}

function projectTreeMarkdown(files = []) {
  const entries = files.filter((file) => file.path || file.name).sort((a, b) => (a.path || a.name).localeCompare(b.path || b.name));
  if (!entries.length) return 'لم يتم العثور على ملفات Workspace ظاهرة في الصفحة.';
  return entries.map((file) => `${file.content ? `\n### ${file.path || file.name}\n\n\`\`\`\n${file.content}\n\`\`\`` : `- ${file.path || file.name}`}`).join('\n');
}

function exportArenaChat() {
  const data = arenaExportData();
  if (!data.messages.length && !data.artifacts.commands.length && !data.artifacts.workspaceFiles.length) {
    showToast('لا يوجد محتوى Arena قابل للتصدير بعد.', { error: true });
    return;
  }
  const format = $('#arena-export-format').value;
  const baseName = (activeProject()?.name || 'arena-project').replace(/[^\p{L}\p{N}_-]+/gu, '-').replace(/^-|-$/g, '') || 'arena-project';
  if (format === 'json') {
    downloadText(`${baseName}-arena.json`, JSON.stringify(data, null, 2), 'application/json;charset=utf-8');
  } else if (format === 'tree') {
    downloadText(`${baseName}-project-tree.md`, `# Project Tree\n\n${projectTreeMarkdown(data.artifacts.workspaceFiles)}`, 'text/markdown;charset=utf-8');
  } else if (format === 'html') {
    const markdown = arenaMarkdown(data);
    downloadText(`${baseName}-arena.html`, `<!doctype html><meta charset="utf-8"><title>Arena export</title><pre>${escapeHtml(markdown)}</pre>`, 'text/html;charset=utf-8');
  } else {
    downloadText(`${baseName}-arena.md`, arenaMarkdown(data), 'text/markdown;charset=utf-8');
  }
  showToast('تم تصدير محادثة Arena والمحتوى المختار.');
}

function arenaMarkdown(data) {
  const lines = [`# ${data.project.name || 'Arena AI Project'}`, '', data.project.brief || '', '', `- المنصة: ${data.provider.name}`, `- الوضع: ${data.provider.mode}`, `- الرابط: ${data.provider.url}`, '', '## المحادثة', ''];
  data.messages.forEach((message) => {
    lines.push(`### ${message.role === 'user' ? 'المستخدم' : 'Arena AI'}${message.lane ? ` · ${message.lane}` : ''}`);
    lines.push('');
    lines.push(message.content);
    lines.push('');
  });
  if (data.artifacts.commands.length) lines.push('## الأوامر', '', '```text', ...data.artifacts.commands, '```', '');
  if (data.artifacts.executions.length) lines.push('## التنفيذ والكونسول', '', '```text', ...data.artifacts.executions, '```', '');
  if (data.artifacts.thinking.length) lines.push('## ملخصات التفكير', '', ...data.artifacts.thinking.map((item) => `- ${item}`), '');
  lines.push('## شجرة المشروع', '', projectTreeMarkdown(data.artifacts.workspaceFiles));
  return lines.join('\n');
}
...
async function sendMessage(value) {
  ...
  try {
    ...
    const response = await agent.reply({
      ...
      aiChatContext: state.aiChatState?.supported
        ? { ...state.aiChatState, artifacts: state.aiChatState.provider === 'arena' ? filteredArenaArtifacts(state.aiChatState.artifacts) : null }
        : project.externalChat,
      additionalAiChatContexts: state.additionalAiChats,
...
async function saveSettingsFromForm() {
  state.settings = normalizeSettings({
    ...
    qualityMode: $('#quality-mode').checked,
...
function handleClick(event) {
  ...
  if (action === 'collect-ai-tabs') collectOpenAiChats();
  if (action === 'arena-attach') $('#arena-file-input').click();
  if (action === 'refresh-arena-artifacts') refreshArenaArtifacts();
  if (action === 'export-ai-chat') exportArenaChat();
...
function bindEvents() {
  ...
  $('#arena-file-input').addEventListener('change', (event) => {
    attachFilesToArena(event.target.files);
    event.target.value = '';
  });
  const arenaDropzone = $('#arena-dropzone');
  arenaDropzone.addEventListener('dragover', (event) => {
    event.preventDefault();
    arenaDropzone.classList.add('dragging');
  });
  arenaDropzone.addEventListener('dragleave', () => arenaDropzone.classList.remove('dragging'));
  arenaDropzone.addEventListener('drop', (event) => {
    event.preventDefault();
    arenaDropzone.classList.remove('dragging');
    attachFilesToArena(event.dataTransfer.files);
  });
  $('#arena-include-commands').addEventListener('change', (event) => { state.arenaCapture.commands = event.target.checked; });
  $('#arena-include-executions').addEventListener('change', (event) => { state.arenaCapture.executions = event.target.checked; });
  $('#arena-include-thinking').addEventListener('change', (event) => { state.arenaCapture.thinking = event.target.checked; });
