...
function renderAiBridge() {
  ...
  if (arenaFileStatus) arenaFileStatus.textContent = state.arenaFiles.length
...
async function filesToPayload(fileList) {
  ...
  for (const file of files) {
    if (file.size > 15 * 1024 * 1024) {
      showToast(`تم تجاهل ${file.name}: الحد الأقصى 15MB.`, { error: true });
      continue;
    }
    ...
    payload.push({ name: file.name, type: file.type, data: btoa(binary), size: file.size });
  }
  return payload;
}
...
async function attachFilesToArena(fileList) {
  ...
  renderAiBridge();
  try {
    const result = await chrome.tabs.sendMessage(state.activeTabId, { type: 'AI_CHAT_ATTACH_FILES', files: payload });
    if (!result?.ok) throw new Error(result?.reason || 'لم يقبل Arena الملفات.');
    state.arenaFiles = [...state.arenaFiles, ...payload.map((file) => ({ name: file.name, size: file.size }))].slice(-8);
    showToast(`تم إرسال ${payload.length} ملفات إلى Arena AI.`);
  } catch (error) {
