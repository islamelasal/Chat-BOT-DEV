...
function normalizeExternalChat(chat) {
  ...
  const messages = Array.isArray(chat.messages)
    ? chat.messages
      ...
      .map((message) => ({ role: message.role, lane: String(message.lane || '').slice(0, 100), content: String(message.content || '').slice(0, 10000) }))
  ...
  return {
    ...
    messages,
    artifacts: chat.artifacts && typeof chat.artifacts === 'object'
      ? {
        commands: Array.isArray(chat.artifacts.commands) ? chat.artifacts.commands.map(String).slice(-30) : [],
        executions: Array.isArray(chat.artifacts.executions) ? chat.artifacts.executions.map(String).slice(-30) : [],
        thinking: Array.isArray(chat.artifacts.thinking) ? chat.artifacts.thinking.map(String).slice(-15) : [],
        workspaceFiles: Array.isArray(chat.artifacts.workspaceFiles)
          ? chat.artifacts.workspaceFiles.slice(-200).map((file) => ({
            name: String(file.name || '').slice(0, 300),
            path: String(file.path || '').slice(0, 1000),
...
function normalizeProject(project) {
  ...
  return {
    ...
    externalChats: Array.isArray(project.externalChats)
      ? project.externalChats.map(normalizeExternalChat).filter(Boolean).slice(0, 8)
      : [],
