...
export const DEFAULT_SETTINGS = Object.freeze({
  ...
  // External grounding tools are opt-in so a free key does not spend quota
  // unexpectedly. Local page context remains enabled by default.
  webSearch: false,
  urlContext: false,
  codeExecution: false,
  streaming: true,
...
export function normalizeSettings(settings = {}) {
  ...
  const isLegacySettings = Number(settings.settingsVersion || 0) < DEFAULT_SETTINGS.settingsVersion;
  ...
  return {
    ...
    thinkingLevel,
    // The previous build enabled server-side tools automatically. Migrate
    // once to a free-tier-safe default; users can enable them explicitly.
